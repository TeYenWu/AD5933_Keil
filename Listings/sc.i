...
# 12 "Library\\StdDriver\\src\\sc.c"



static uint32_t u32CardStateIgnore[1] = {0};
...
uint32_t SC_IsCardInserted(SC_T *sc)
{
    ...
    uint32_t cond1 = ((sc->STATUS & (0x1ul << (13))) >> (13));
    uint32_t cond2 = ((sc->CTL & (0x1ul << (26))) >> (26));

    if(sc == ((SC_T *) (((0x40000000UL) + 0x00040000) + 0x50000)) && u32CardStateIgnore[0] == 1)
        return 1;
# 59 "Library\\StdDriver\\src\\sc.c"
    else if(cond1 != cond2)
        return 0;
    else
        return 1;
...
void SC_Open(SC_T *sc, uint32_t u32CD, uint32_t u32PWR)
{
    ...
    if(sc == ((SC_T *) (((0x40000000UL) + 0x00040000) + 0x50000)))
        u32Intf = 0;
# 121 "Library\\StdDriver\\src\\sc.c"
    else
        return ;

    if(u32CD != 0xFFFFFFFF) {
        u32Reg = u32CD ? 0: (0x1ul << (26));
        u32CardStateIgnore[u32Intf] = 0;
    } else {
        u32CardStateIgnore[u32Intf] = 1;
    }
    while(sc->PINCTL & (0x1ul << (30)));
    sc->PINCTL = u32PWR ? 0 : (0x1ul << (11));
    while(sc->CTL & (0x1ul << (30)));
    sc->CTL = (0x1ul << (0)) | u32Reg;
}
...
void SC_ResetReader(SC_T *sc)
{
    ...
    if(sc == ((SC_T *) (((0x40000000UL) + 0x00040000) + 0x50000)))
        u32Intf = 0;
# 160 "Library\\StdDriver\\src\\sc.c"
    else
        return ;

	
    sc->ALTCTL |= ((0x1ul << (0)) | (0x1ul << (1)) | (0x1ul << (11)));
    
    while(sc->CTL & (0x1ul << (30)));
    sc->CTL &= ~((0x3ul << (6)) | (0x3ul << (24)) | (0x7ul << (20)) | (0x7ul << (16)));
    
    sc->CTL |= (0x1ul << (3)) | (0x3ul << (13));
...
void SC_SetBlockGuardTime(SC_T *sc, uint32_t u32BGT)
{
    sc->CTL = (sc->CTL & ~(0x1ful << (8))) | ((u32BGT - 1) << (8));
}
...
void SC_StopAllTimer(SC_T *sc)
{
    sc->ALTCTL &= ~((0x1ul << (5)) | (0x1ul << (6)) | (0x1ul << (7)));
}
...
void SC_StartTimer(SC_T *sc, uint32_t u32TimerNum, uint32_t u32Mode, uint32_t u32ETUCount)
{
    uint32_t reg = u32Mode | ((0xfffffful << (0)) & (u32ETUCount - 1));
