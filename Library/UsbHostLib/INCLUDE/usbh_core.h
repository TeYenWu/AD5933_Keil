...
/* USB Host Power auto control enable */
...
typedef struct
{
    ...
    __packed uint8_t  request;          /*!< Specific request                       */
    __packed uint16_t value;            /*!< Word-sized field that varies according to request */
 ...
 */
typedef struct urb_t                    /*!< URB structure                          */
{
    URB_PRIV_T  urb_hcpriv;             /*!< private data for host controller       */
    USB_LIST_T  urb_list;               /*!< list pointer to all active urbs        */
    struct urb_t  *next;                /*!< pointer to next URB                    */
    ...
    void (*complete)(struct urb_t *);   /*!< USB transfer complete callback function*/
    ISO_PACKET_DESCRIPTOR_T  iso_frame_desc[8]; /*!< isochronous transfer descriptor  */
}   URB_T;                              /*!< URB structure                          */
 ...
 */
typedef struct usb_device               /*!< USB device structure                   */
{
    USB_DEV_DESC_T  descriptor;         /*!< Device descriptor.                     */

    int     devnum;                     /*!< Device number on USB bus               */
    int     slow;                       /*!< Is slow device.                        */
    int     speed;                      /*!< Device speed.                          */

    uint32_t    toggle[2];              /*!< toggle bit ([0] = IN, [1] = OUT)       */
    uint32_t    halted[2];              /*!< endpoint halts;  one bit per endpoint # & direction; */
    /* [0] = IN, [1] = OUT */

    struct usb_device  *parent;         /*!< parent device                          */
    int                 hub_port;       /*!< The hub port that this device connected on */
    USB_BUS_T  *bus;                    /*!< Bus we're part of */

    signed char act_config;             /*!< Active configuration number            */
    char        act_iface;              /*!< Active interface number                */
    char        iface_alternate;        /*!< Active interface alternate setting     */

    EP_INFO_T   ep_list[MAX_ENDPOINTS]; /*!< Endpoint list                          */
    int         ep_list_cnt;            /*!< Total number of ep_list[]              */
    ...
    struct usb_device  *children[USB_MAXCHILDREN]; /*!< Child device list          */

    USB_DRIVER_T    *driver[MAX_DRIVER_PER_DEV]; /*!< Driver list                  */
    int             driver_cnt;        /*!< Total number of driver list            */

} USB_DEV_T;                           /*!< USB device structure                   */
